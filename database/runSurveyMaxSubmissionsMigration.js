...
/**
 * Apply database/migration_survey_max_submissions.sql (max_submissions, total_submissions).
 *
 * @returns {Promise<{ executed: number }>}
 */
async function runSurveyMaxSubmissionsMigration() {
	const { getPool, parsePgError, formatDbErrorForLog } = require('../db.js');
	const pool = getPool();
	const sqlPath = path.join(__dirname, 'migration_survey_max_submissions.sql');
	...
	try {
		...
		throw new Error(
			[
				`[db:migrate] Statement ${executed + 1}/${statements.length} failed:`,
				message,
				code ? `code: ${code}` : '',
				`snippet: ${snippet}`,
...
module.exports = { runSurveyMaxSubmissionsMigration };
