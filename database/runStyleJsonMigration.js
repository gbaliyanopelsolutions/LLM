...
/**
 * Apply database/migration_add_style_json.sql
 * Adds style_json column to surveys table for storing form theme/styling data
 *
 * @returns {Promise<{ executed: number }>}
 */
async function runStyleJsonMigration() {
	const { getPool, parsePgError, formatDbErrorForLog } = require('../db.js');
	const pool = getPool();
	const sqlPath = path.join(__dirname, 'migration_add_style_json.sql');
	...
	try {
		...
		throw new Error(
			[
				`[db:migrate:style-json] Statement ${executed + 1}/${statements.length} failed:`,
				message,
				code ? `code: ${code}` : '',
				`snippet: ${snippet}`,
...
module.exports = { runStyleJsonMigration };
