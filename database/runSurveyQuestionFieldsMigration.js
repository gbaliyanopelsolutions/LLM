...
/**
 * Apply database/migration_survey_question_fields.sql
 *
 * @returns {Promise<{ executed: number }>}
 */
async function runSurveyQuestionFieldsMigration() {
	const { getPool, parsePgError, formatDbErrorForLog } = require('../db.js');
	const pool = getPool();
	const sqlPath = path.join(__dirname, 'migration_survey_question_fields.sql');
	...
	try {
		...
		throw new Error(
			[
				`[db:migrate:question-fields] Statement ${executed + 1}/${statements.length} failed:`,
				message,
				code ? `code: ${code}` : '',
				`snippet: ${snippet}`,
...
module.exports = { runSurveyQuestionFieldsMigration };
