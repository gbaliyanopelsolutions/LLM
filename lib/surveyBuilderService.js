...
{ "question": "...", "type": "...", "required": true, "options": ["for radio/checkbox/select only"], "page": 1 }

PAGES / STEPS:
- Add a "page" integer (starting at 1) to every question.
- Group logically related questions on the same page (e.g. same survey block, or same theme).
- Matrix questions should always be on their own page.
- Simple intro / welcome questions can share a page.
- Aim for 3-6 pages total for surveys with 10+ questions.
- Example: pages 1 (intro), 2 (experience), 3 (ratings matrix), 4 (overall feedback), 5 (demographics).
...
CLEANING RULES (apply to all questions, options, rows):
- Strip leading question-number prefixes: "Q0 ", "Q1 ", "Q2.", "Q10 " etc.
- Strip trailing Qualtrics value labels: " (1)", " (2)", " (11)" etc.
- Example: "Q6 How satisfied..." → "How satisfied..."
- Example: "Online Ad  (1)" → "Online Ad"
- Example: "Purchase Process (6)" → "Purchase Process"

QUALTRICS MATRIX PATTERN — critical for Qualtrics-exported documents:
When you see a question block like:
  "Q10 How satisfied are you with X in terms of the following:
   Not at All Satisfied (0) (1)  1 (2)  2 (3) ... Extremely Satisfied (10) (11)
   Purchase Process (6)
   Agent Interactions (7)
   ..."
The SCALE header row (Not at All / numbers / Extremely) defines the numeric scale (min=0, max=10).
The items AFTER the scale header (Purchase Process, Agent Interactions, etc.) are the matrix ROWS.
DO NOT include scale labels as rows. DO include only the real sub-items.
Generate as: { "type": "matrix_rating", "scale": { "min": 0, "max": 10 }, "rows": ["Purchase Process", "Agent Interactions", ...] }

PRE-DETECTED MATRIX BLOCK — when the prompt includes a "PRE-DETECTED MATRIX QUESTIONS" block:
ALWAYS use those exact rows, that exact scale, and the question text provided there.
Do NOT re-infer the scale or rows from the raw document text.
These are guaranteed accurate because they were parsed directly from the DOCX table structure.

STYLE — read ONLY the REQUIREMENTS section for visual rules (not the document content):
Accepted formats (any of these mean the same thing):
  backgroundColor: "background color #xyz" | "bg #xyz" | "backgroundColor: '#xyz'" | "backgroundColor = '#xyz'" | "s.backgroundColor = '#xyz'" | "backgroundColor: #xyz"
  textColor: "text color #xyz" | "font color #xyz" | "textColor: '#xyz'" | "textColor = '#xyz'" | "s.textColor = '#xyz'"
  accentColor: "accent color #xyz" | "accentColor: '#xyz'" | "s.accentColor = '#xyz'"
  logoUrl: "logo ... https://..." | "logoUrl: 'https://...'" | "logoUrl = 'https://...'" | "s.logoUrl = 'https://...'" | "logo image url https://..."
Only include style fields that are EXPLICITLY mentioned. Do NOT invent colors.

STYLE EXAMPLE — for requirements: "backgroundColor: '#999', textColor: '#fff', logoUrl: 'https://example.com/logo.png'"
"style": { "backgroundColor": "#999", "textColor": "#fff", "logoUrl": "https://example.com/logo.png" }

Return format:
{
  "title": "Survey title",
  "description": "Brief description",
  "style": { "backgroundColor": "#999", "textColor": "#fff", "logoUrl": "https://..." },
  "questions": [ ...question objects... ]
}
IMPORTANT: Always include the "style" field when the requirements mention any color or logo.`;
...
async function listCompanies() {
	...
	try {
		const pool = getPool();
		const { rows } = await pool.query(
			`SELECT company_id, name, industry, region, tier, metadata, created_at
...
async function createCompany(body) {
	...
	try {
		const pool = getPool();
		const metadataObj = (b && typeof b.metadata === 'object') ? b.metadata : {};
		const { rows } = await pool.query(
			`INSERT INTO public.companies (name, industry, region, tier, metadata)
			 VALUES ($1, $2, $3, $4, $5::jsonb)
			 RETURNING company_id, name, industry, region, tier, metadata, created_at`,
			[name, industry || null, region || null, tierRaw, JSON.stringify(metadataObj)]
		);
...
async function getCompanyById(companyId) {
	...
	if (!companyId || typeof companyId !== 'string') {
		return { status: 400, json: { ok: false, error: 'Company ID is required', code: 'VALIDATION' } };
	}
	try {
		const pool = getPool();
		const { rows } = await pool.query(
		...
		return { status: 200, json: { ok: true, company: rows[0] } };
	} catch (err) {
		const { message, code } = parsePgError(err);
		return { status: 500, json: { ok: false, error: message, code: code || 'DB_ERROR' } };
	}
...
async function updateCompany(companyId, body) {
	...
	const industry = typeof b.industry === 'string' ? b.industry.trim() : null;
	const region = typeof b.region === 'string' ? b.region.trim() : null;
	const tierRaw = typeof b.tier === 'string' ? b.tier.trim() : null;
	const allowedTiers = new Set(['Tier 1', 'Tier 2', 'Tier 3']);

	if (name !== null && !name) {
		return { status: 400, json: { ok: false, error: 'Company name cannot be empty', code: 'VALIDATION' } };
	}
	if (tierRaw !== null && !allowedTiers.has(tierRaw)) {
		return { status: 400, json: { ok: false, error: 'Tier must be Tier 1, Tier 2, or Tier 3', code: 'VALIDATION' } };
	}

	try {
		const pool = getPool();
		const metadataObj = (b && typeof b.metadata === 'object') ? b.metadata : undefined;
...
/**
 * Delete company by ID (checks for referenced surveys first)
 * @param {string} companyId
 * @returns {Promise<{ status: number, json: object }>}
 */
async function deleteCompany(companyId) {
	...
	try {
		...
		return { status: 200, json: { ok: true, message: 'Company deleted' } };
	} catch (err) {
		const { message, code } = parsePgError(err);
		return { status: 500, json: { ok: false, error: message, code: code || 'DB_ERROR' } };
	}
...
async function generateSurveyJson({ anthropic, model, body }) {
	...
	try {
		...
		// Accept both full survey objects (title + questions) and partial updates (style/title/description)
		const isFullSurvey = parsed && typeof parsed.title === 'string' && Array.isArray(parsed.questions);
		const isPartialUpdate = parsed && (parsed.style || parsed.title || parsed.description);

		if (!isFullSurvey && !isPartialUpdate) {
...
async function saveSurvey(body) {
	...
	try {
		...
		const styleData = (survey && typeof survey.style === 'object') ? survey.style : null;
		const insertParams = [
			title,
			desc,
			'llm-builder',
			...
			styleData ? JSON.stringify(styleData) : null,
		];
		let insertSql = `INSERT INTO public.surveys (name, description, category, status, company_id, settings_json, style_json)
			 VALUES ($1, $2, $3, $4::public.survey_status, $5::uuid, $6::jsonb, $7::jsonb)
			 RETURNING survey_id`;
		if (cols.hasMax) {
			insertSql = `INSERT INTO public.surveys (name, description, category, status, company_id, settings_json, style_json, max_submissions)
			 VALUES ($1, $2, $3, $4::public.survey_status, $5::uuid, $6::jsonb, $7::jsonb, $8)
			 RETURNING survey_id`;
...
async function submitSurvey(surveyId, body) {
	...
	if (entries.length === 0) {
		return { status: 400, json: { ok: false, error: 'Please answer at least one question', code: 'VALIDATION' } };
	}
...
/**
 * Duplicate a survey with all its questions.
 *
 * @param {string} surveyId
 * @returns {Promise<{ status: number, json: object }>}
 */
async function duplicateSurvey(surveyId) {
	...
	try {
		...
		await pool.query(
			...
			[
				newSurveyId,
				originalSurvey.name + ' (Copy)',
				originalSurvey.description,
				'draft',
				originalSurvey.category,
				originalSurvey.company_id,
				originalSurvey.form_html,
				originalSurvey.form_css,
				originalSurvey.settings_json,
				originalSurvey.max_submissions,
				timestamp,
				timestamp
			]
		);
		...
		return { status: 201, json: { ok: true, surveyId: newSurveyId } };
	} catch (err) {
		const { message, code } = parsePgError(err);
		return { status: 500, json: { ok: false, error: message, code: code || 'DB_ERROR' } };
	}
