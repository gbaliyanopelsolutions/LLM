...
const JSON_SYSTEM = `You are a survey schema assistant. Output ONLY a single JSON object (no markdown fences, no commentary).
...
Required JSON shape:
{
  ...
  "questions": [
    {
      "question": "string",
      "type": "text|textarea|radio|checkbox|number|date|likert|rating|select",
      "options": ["optional", "for radio checkbox select"],
      "required": true
    }
  ]
}
...
- Types: prefer text, radio, checkbox, number, date, likert, rating, select.
- For radio, checkbox, or select, include "options" array with at least 2 strings.
- For likert or rating, you may omit options or provide labels in options.`;
...
async function generateSurveyJson({ anthropic, model, body }) {
	...
	let userBlock = `Produce the survey JSON for this request:\n${prompt}`;
	if (htmlSample) {
		userBlock += `\n\nHTML form sample (extract fields if helpful):\n${htmlSample}`;
	}
	const messages = [...msgs, { role: 'user', content: userBlock }];
