...
/**
 * Rebind data-question-id attributes in stored form HTML to the real DB question UUIDs.
 *
 * The saved HTML may contain editor-local UUIDs in data-question-id. This function
 * replaces them with actual DB question_ids by matching unique editor UUIDs in their
 * order of first appearance against the ordered qRows array. All occurrences of the
 * same editor UUID (e.g. all radios in a group) get the same DB UUID.
 *
 * This is called at serve time so existing surveys are automatically fixed without
 * requiring a re-save.
 *
 * @param {string} html
 * @param {{ question_id: string }[]} questions  Ordered by sort_order ASC
 * @returns {string}
 */
function rebindDataQuestionIds(html, questions) {
	if (!html || !questions || !questions.length) return html;
	/** @type {Map<string, string>} editorId → dbId */
	const map = new Map();
	let idx = 0;
	return html.replace(/\bdata-question-id="([^"]*)"/g, (_m, editorId) => {
		if (!map.has(editorId)) {
			const dbId = idx < questions.length ? questions[idx++].question_id : editorId;
			map.set(editorId, dbId);
		}
		return `data-question-id="${map.get(editorId)}"`;
	});
}
